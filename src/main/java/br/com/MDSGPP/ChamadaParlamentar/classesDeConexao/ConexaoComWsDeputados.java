...
public class ConexaoComWsDeputados {
	...
	/**
	 * This method is to parse the data from the webService to JavaObjects to it
	...
	public static ArrayList<Deputados> criaLista()
		...
		NodeList partido = deputados.get_any()[0]
				.getElementsByTagName("partido");
		NodeList gabinete = deputados.get_any()[0]
		...
		for (int i = 0; i < nome.getLength(); i++) {
			...
			try {// esse try catch esta aqui por causa de um erro oriundo do
					// webservice no qual
			...
			Deputados deputadoNovo = new Deputados(idParlamentarInt,
					matriculaInt, idInt, nomeText, nomeTratamentoText,
					sexoText, ufText, partidoText, gabineteText, anexoText,
					foneText, emailText);
	...
	public static ObterPartidosCDResponseObterPartidosCDResult receberElementPartido(
		...
		// conexao criada, agora chamaremos a classe do ws
		try {
			ObterPartidosCDResponseObterPartidosCDResult partidos = service
					.obterPartidosCD();
	...
	public static ArrayList<Partidos> criaListaPartidos()
		...
		NodeList nomePartido = partidos.get_any()[0]
				.getElementsByTagName("nomePartido");
		NodeList siglaPartido = partidos.get_any()[0]
				.getElementsByTagName("siglaPartido");
		NodeList dataExtincao = partidos.get_any()[0]
				.getElementsByTagName("dataExtincao");

		for (int i = 0; i < nomePartido.getLength(); i++) {
			MessageElement nomePartidoTemp = (MessageElement) nomePartido
					.item(i);
			MessageElement siglaPartidoTemp = (MessageElement) siglaPartido
					.item(i);
			MessageElement dataExtincaoTemp = (MessageElement) dataExtincao
					.item(i);

			String nomePartidoText = nomePartidoTemp.getFirstChild()
					.getNodeValue();
			String siglaPartidoText = siglaPartidoTemp.getFirstChild()
					.getNodeValue();

			// esse try catch é para retirar os partidos que já nao
			// existem mais
			try {
				String dataExtincaoText = dataExtincaoTemp.getFirstChild()
						.getNodeValue();
			} catch (NullPointerException e) {
